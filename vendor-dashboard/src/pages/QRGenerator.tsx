...
export default function QRGenerator() {
  ...
  const completeTransaction = async () => {
    if (!qrData?.orderId) {
      // If no orderId, just close the QR modal
      setShowQR(false);
      setQrData(null);
      setCart([]);
      return;
    ...
    try {
      const response = await api.post('/api/vendor/complete-qr-order', {
        orderId: qrData.orderId,
      });

      // Success - close QR modal, clear cart and QR data
      setShowQR(false);
      setQrData(null);
      setCart([]);
      
      // Show success message
      alert('Transaction completed successfully! Amount added to your balance.');
      ...
      // If order already completed or transaction exists, just close the modal
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="rounded-lg p-8 max-w-md w-full relative" style={{ backgroundColor: colors.white }}>
            <button
              ...
              title="Complete transaction and close"
            >
              <X size={24} />
            </button>
