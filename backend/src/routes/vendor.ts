...
// Complete QR order (manual completion when vendor closes QR code)
const completeQROrderSchema = z.object({
  orderId: z.string().uuid(),
});

router.post('/complete-qr-order', async (req: AuthRequest, res) => {
  try {
    const { orderId } = completeQROrderSchema.parse(req.body);
    ...
    if (existingTransaction) {
      return res.status(400).json({ error: 'Transaction already exists for this order' });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const updatedVendorWallet = await tx.wallet.update({
        ...
        data: {
          btcBalance: {
            increment: order.totalBtc,
          },
          sbtcBalance: {
            increment: order.totalSbtc,
          },
        },
      });
      ...
      // For manual completion, we set senderId = receiverId = vendorId (similar to ADD_FUNDS)
      const transaction = await tx.transaction.create({
        data: {
          senderId: req.userId!,
          receiverId: req.userId!,
          amountBtc: order.totalBtc,
          amountSbtc: order.totalSbtc,
          type: 'PAYMENT',
          status: 'COMPLETED',
          orderId: order.id,
          description: `Manual payment completion for order ${order.id}`,
        },
        include: {
          sender: {
            select: {
      ...
      return { updatedOrder, transaction, updatedVendorWallet };
    });
    ...
    res.json({
      ...
      newBalance: {
        btc: parseFloat(formatBtc(result.updatedVendorWallet.btcBalance)),
        sbtc: parseFloat(formatSbtc(result.updatedVendorWallet.sbtcBalance)),
        btcFormatted: formatBtc(result.updatedVendorWallet.btcBalance),
        sbtcFormatted: formatSbtc(result.updatedVendorWallet.sbtcBalance),
      },
    });
  } catch (error: any) {
    if (error instanceof z.ZodError) {
